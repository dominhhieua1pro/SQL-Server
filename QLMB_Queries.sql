...
Select MAYBAY.MaMB, Hieu, Count(Distinct NHANVIEN.MaNV) as 'Total Number of Pilots' 
...
Select a.MaCB From CHUYENBAY as a, CHUYENBAY as b
...
Select GaDi, Count(GioDi) as 'SL CB XP Truoc 12h' From CHUYENBAY
Where GioDi < '12:00:00'
Group By GaDi

--21.	Cho biết mã số của phi công chỉ lái được 3 loại máy bay
Select NHANVIEN.MaNV From NHANVIEN, CHUNGNHAN
...
Select NHANVIEN.MaNV, max(TamBay) as'Tam Bay Cao Nhat' From NHANVIEN, CHUNGNHAN, MAYBAY
Where NHANVIEN.MaNV = CHUNGNHAN.MaNV and CHUNGNHAN.MaMB = MAYBAY.MaMB
Group By NHANVIEN.MaNV
...
Select Sum(Luong) as 'Tong Luong Phi Cong' From NHANVIEN, CHUNGNHAN
Where NHANVIEN.MaNV = CHUNGNHAN.MaNV
...
Where TamBay >= (Select DoDai From CHUYENBAY Where GaDi = 'SGN' and GaDen = 'HUI')

--30.	Tìm các chuyến bay có thể được lái bởi các phi công có lương lớn hơn 100.000
Select * From CHUYENBAY
Where DoDai <= Some(Select Distinct TamBay From NHANVIEN, CHUNGNHAN, MAYBAY
...
Where Luong < All(Select ChiPhi From CHUYENBAY Where GaDi = 'SGN' and GaDen = 'BMV')
...
Where MaNV in ( 
		...
		Where Luong >  ( Select Avg(Luong) 
						 From NHANVIEN Where MaNV in (	Select Distinct NHANVIEN.MaNV From NHANVIEN, CHUNGNHAN 
														Where NHANVIEN.MaNV = CHUNGNHAN.MaNV ) )
		Except				
		Select MaNV From CHUNGNHAN )				 
...
Select NHANVIEN.MaNV, Ten, Count(Distinct MaMB) as 'Tong so MB' 
...
Select NHANVIEN.MaNV,Ten, Count(a.MaNV) as 'Tong so MB Boeing' 
From (	Select MaNV 
...
Select mb.MaMB, Hieu, Count(nv.MaNV) as 'Tong so PC' 
...
Select Hieu, Count(mb.MaMB) 'So Luong PC' 
From (NHANVIEN nv join CHUNGNHAN cn On nv.MaNV = cn.MaNV) join MAYBAY mb On cn.MaMB = mb.MaMB
Where Luong > 100000
Group By Hieu
...
-- Cách 1: Except
Select a.Hieu, Count(Distinct a.MaNV) as SLNV
From (	Select Hieu, NHANVIEN.MaNV From NHANVIEN, CHUNGNHAN, MAYBAY
		Except
		Select Hieu, NHANVIEN.MaNV From NHANVIEN, CHUNGNHAN, MAYBAY
		Where NHANVIEN.MaNV = CHUNGNHAN.MaNV and CHUNGNHAN.MaMB = MAYBAY.MaMB ) as a
Group By a.Hieu
...
--Cách 3: subtract
Select Hieu, ((Select Count(MaNV) From NHANVIEN) - Count(nv.MaNV)) as SLNV
From (NHANVIEN nv join CHUNGNHAN cn On nv.MaNV = cn.MaNV) join MAYBAY mb On cn.MaMB = mb.MaMB
Group By Hieu

--45.	Với mỗi loại máy bay  hãy cho biết loại máy bay và tổng số phi công không thể lái loại máy bay đó.
--Cách 1: Except
Select a.Hieu, Count(Distinct a.MaNV) as SLPC
From (	Select Hieu, NHANVIEN.MaNV From NHANVIEN, CHUNGNHAN, MAYBAY
		Where NHANVIEN.MaNV = CHUNGNHAN.MaNV
		Except
		Select Hieu, NHANVIEN.MaNV From NHANVIEN, CHUNGNHAN, MAYBAY
...
From (	Select MaNV, MaCB
		From NHANVIEN nv, CHUYENBAY cb
...
--Cách 2:
Select NHANVIEN.MaNV, Ten, Count(Distinct MaCB) as N'Số CB Có Thể Lái' 
From ((NHANVIEN join CHUNGNHAN On NHANVIEN.MaNV = CHUNGNHAN.MaNV) join MAYBAY On CHUNGNHAN.MaMB = MAYBAY.MaMB) 
	 left join CHUYENBAY On TamBay >= DoDai and GaDi = 'SGN'																
Group By NHANVIEN.MaNV, Ten
...
From (	Select NHANVIEN.MaNV, Max(TamBay) as Max_TB 
		From ((NHANVIEN join CHUNGNHAN On NHANVIEN.MaNV = CHUNGNHAN.MaNV) join MAYBAY On CHUNGNHAN.MaMB = MAYBAY.MaMB) 
		Group By NHANVIEN.MaNV ) as a right join CHUYENBAY On Max_TB < DoDai
Group By MaCB

--Note: cách sau đây không đúng
Select MaCB, Count(Distinct NHANVIEN.MaNV) as N'Số PC Không Thể Lái' 
From ((NHANVIEN join CHUNGNHAN On NHANVIEN.MaNV = CHUNGNHAN.MaNV) join MAYBAY On CHUNGNHAN.MaMB = MAYBAY.MaMB) right join CHUYENBAY On TamBay < DoDai 															
Group By MaCB
...
Where MaCB in (	Select MaCB 
				From (	Select nv.MaNV, Ten, Luong, Max(TamBay) as Max_TB 
						From ((NHANVIEN nv join CHUNGNHAN cn On nv.MaNV = cn.MaNV) join MAYBAY mb On cn.MaMB = mb.MaMB)  
						Group By nv.MaNV, Ten, Luong ) as a join CHUYENBAY On Max_TB >= DoDai
				Group By MaCB
